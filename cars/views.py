...
class ListCars(ListView):
    ...
    def get_queryset(self):
        ...
        return Car.objects.filter(user_id=user, is_deleted=False).prefetch_related('car_photo')
...
class EditCar(UpdateView):
    ...
    def form_valid(self, form, ):  # TODO Реализовать удаление старой фотографии при создании новой
        self.object = form.save(commit=False)
        if self.request.FILES:
            CarPhoto.objects.update_or_create(car=self.object, photo=self.request.FILES['car_photo'])
        self.object.save()
